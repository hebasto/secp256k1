...
#define secp256k1_fe_impl_negate(r, a, m) ASSERT_INT_CONST_AND_DO(m, secp256k1_fe_impl_negate_unchecked(r, a, m))
...
SECP256K1_INLINE static int secp256k1_fe_impl_equal(const secp256k1_fe *a, const secp256k1_fe *b) {
    secp256k1_fe na;
#ifdef VERIFY
    secp256k1_fe_impl_verify(a);
    secp256k1_fe_impl_verify(b);
...
#endif
    secp256k1_fe_impl_negate(&na, a, 1);
    secp256k1_fe_impl_add(&na, b);
    return secp256k1_fe_impl_normalizes_to_zero(&na);
}

SECP256K1_INLINE static int secp256k1_fe_impl_equal_var(const secp256k1_fe *a, const secp256k1_fe *b) {
    secp256k1_fe na;
#ifdef VERIFY
    secp256k1_fe_impl_verify(a);
    secp256k1_fe_impl_verify(b);
...
static int secp256k1_fe_impl_sqrt(secp256k1_fe *r, const secp256k1_fe *a) {
    ...
    secp256k1_fe_impl_sqr(&x2, a);
    secp256k1_fe_impl_mul(&x2, &x2, a);

    secp256k1_fe_impl_sqr(&x3, &x2);
    secp256k1_fe_impl_mul(&x3, &x3, a);

    x6 = x3;
    for (j=0; j<3; j++) {
        secp256k1_fe_impl_sqr(&x6, &x6);
    }
    secp256k1_fe_impl_mul(&x6, &x6, &x3);

    x9 = x6;
    for (j=0; j<3; j++) {
        secp256k1_fe_impl_sqr(&x9, &x9);
    }
    secp256k1_fe_impl_mul(&x9, &x9, &x3);

    x11 = x9;
    for (j=0; j<2; j++) {
        secp256k1_fe_impl_sqr(&x11, &x11);
    }
    secp256k1_fe_impl_mul(&x11, &x11, &x2);

    x22 = x11;
    for (j=0; j<11; j++) {
        secp256k1_fe_impl_sqr(&x22, &x22);
    }
    secp256k1_fe_impl_mul(&x22, &x22, &x11);

    x44 = x22;
    for (j=0; j<22; j++) {
        secp256k1_fe_impl_sqr(&x44, &x44);
    }
    secp256k1_fe_impl_mul(&x44, &x44, &x22);

    x88 = x44;
    for (j=0; j<44; j++) {
        secp256k1_fe_impl_sqr(&x88, &x88);
    }
    secp256k1_fe_impl_mul(&x88, &x88, &x44);

    x176 = x88;
    for (j=0; j<88; j++) {
        secp256k1_fe_impl_sqr(&x176, &x176);
    }
    secp256k1_fe_impl_mul(&x176, &x176, &x88);

    x220 = x176;
    for (j=0; j<44; j++) {
        secp256k1_fe_impl_sqr(&x220, &x220);
    }
    secp256k1_fe_impl_mul(&x220, &x220, &x44);

    x223 = x220;
    for (j=0; j<3; j++) {
        secp256k1_fe_impl_sqr(&x223, &x223);
    }
    secp256k1_fe_impl_mul(&x223, &x223, &x3);

    /* The final result is then assembled using a sliding window over the blocks. */

    t1 = x223;
    for (j=0; j<23; j++) {
        secp256k1_fe_impl_sqr(&t1, &t1);
    }
    secp256k1_fe_impl_mul(&t1, &t1, &x22);
    for (j=0; j<6; j++) {
        secp256k1_fe_impl_sqr(&t1, &t1);
    }
    secp256k1_fe_impl_mul(&t1, &t1, &x2);
    secp256k1_fe_impl_sqr(&t1, &t1);
    secp256k1_fe_impl_sqr(r, &t1);

    /* Check that a square root was actually calculated */

    secp256k1_fe_impl_sqr(&t1, r);
    ret = secp256k1_fe_impl_equal(&t1, a);

#ifdef VERIFY
    if (!ret) {
        secp256k1_fe_impl_negate(&t1, &t1, 1);
        secp256k1_fe_impl_normalize_var(&t1);
        VERIFY_CHECK(secp256k1_fe_impl_equal_var(&t1, a));
    }
#endif
    return ret;
}
...
static int secp256k1_fe_impl_is_square_var(const secp256k1_fe *x) {
    ...
    if (jac == 0) {
        ...
        secp256k1_fe dummy;
        ret = secp256k1_fe_impl_sqrt(&dummy, &tmp);
    } else {
        ret = jac >= 0;
    }
    return ret;
